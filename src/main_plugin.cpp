...
bool validateTeams( std::vector< std::vector< double > > &T, int t )
{
	if( t < 2 )
	{
		Plugin_Scr_Error( "There must be atleast 2 teams. in FFA each player counts as a team \n" );
		return false;
	}
	...
	for( unsigned int i = 0; i < T.size(); i++ )
	{
		if( ( int )T[ i ][ 2 ] > t )
		{
			Plugin_Scr_Error( "Amount of teams missmatch. Check if number of teams is correct \n" );
			return false;
		}
	}
